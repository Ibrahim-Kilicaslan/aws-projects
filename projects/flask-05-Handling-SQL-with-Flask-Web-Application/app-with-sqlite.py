...
from flask_sqlalchemy import SQLAlchemy  # kutuphaneleri yukluyor
from sqlalchemy import text
app = Flask(__name__)  # Flask ile birlikte bu geliyor
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///./email.db'  #connfigurations ayarlari yapiliyor. email.db ver tabani
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)

with app.app_context(): # veri tabani tablosu olusturulmus
...
def find_emails(keyword): # email bulma islemi yapacak
...
def insert_email(name,email): # email girme islemi yapacak
    with app.app_context():
        query = text(f"""
...
def emails():
    with app.app_context():
        if request.method == 'POST':
            user_app_name = request.form['user_keyword'] # user ve email girilecek kutucuklalara , yani bir rewuest yapilacak
